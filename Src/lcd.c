...
void SetDataGPIOMode(bool input)
{
		...
		{
			GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;  // 上拉输出
...
*
* Description: 对LCD显示器的某一窗口以某种颜色进行清屏
* Arguments  :           
...
//	a = LCD_ReadReg(0x0000);
	...
	delay_us(30); 
	LCD_WriteReg(0x0010,0x16B0);    // Write final user鈥檚 setting values to BT bits 
	LCD_WriteReg(0x0011,0x0007);    // Write final user鈥檚 setting values to VC bits 
	delay_us(30); 
...
//{
	delay_ms(5);
	LCD_Clear(WHITE);//第一次清屏，数据无效，但是是必需的
	delay_ms(5);
	LCD_Clear(CYAN);
	delay_ms(5);
	LCD_Clear(YELLOW);
...
// 	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable , ENABLE);//开启SWD，失能JTAG
	...
	GPIO_InitStructure.Pin = GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5|GPIO_PIN_6|GPIO_PIN_7|GPIO_PIN_8;	   ///PORTC6~10复用推挽输出
...
//		    //x,y坐标对调 
//	LCD_WriteReg(0x0052,(320-bmp.pic_w_l)/2);//垂直 GRAM起始位置
//	LCD_WriteReg(0x0053,(320-bmp.pic_w_l)/2+bmp.pic_w_l-1);//垂直GRAM终止位置

//	LCD_WriteReg(0x0050,(240-bmp.pic_h_l)/2);//水平GRAM起始位置
//	LCD_WriteReg(0x0051,(240-bmp.pic_h_l)/2+bmp.pic_h_l-1);//水平GRAM终止位置 
//	Lcd_WR_Start();
//	}
////     else 
//	ty=0;
