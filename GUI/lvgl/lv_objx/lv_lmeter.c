 ...
 *   STATIC FUNCTIONS
 **********************/
static void lv_lmeter_draw_scale(lv_obj_t * lmeter,const lv_area_t *mask)
{
	lv_lmeter_ext_t * ext = lv_obj_get_ext_attr(lmeter);
	...
	lv_coord_t w = lv_obj_get_width(lmeter) - style_tmp.line.width - 4;
	lv_coord_t h = lv_obj_get_height(lmeter) - style_tmp.line.width - 4; 
	
	lv_area_t mycoords; 
	mycoords.x1 = (lmeter->coords.x1 + style_tmp.line.width + 4);
	mycoords.y1 = (lmeter->coords.y1 + style_tmp.line.width + 4);
	mycoords.x2 = (lmeter->coords.x2 - style_tmp.line.width - 4);
	mycoords.y2 = (lmeter->coords.y2 - style_tmp.line.width - 4);
	
	uint8_t my_line_cnt;
	my_line_cnt = ext->line_cnt*2; 
	
	lv_coord_t r_out = w / 2;
	lv_coord_t r_in = r_out - style->body.padding.hor;
	if(r_in < 1) r_in = 1;

	lv_coord_t x_ofs = w / 2 + mycoords.x1;
	lv_coord_t y_ofs = h / 2 + mycoords.y1;
	int16_t angle_ofs = 90 + (360 - ext->scale_angle) / 2;
	int16_t level = (int32_t)((int32_t)(ext->cur_value - ext->min_value) * my_line_cnt) / (ext->max_value - ext->min_value);
	uint8_t i;
	...
	for(i = 0; i < my_line_cnt; i++) {
			/*Calculate the position a scale label*/
			int16_t angle = (i * ext->scale_angle) / (my_line_cnt - 1) + angle_ofs;
			...
			{
				style_tmp.line.color = lv_color_mix(style->body.grad_color, style->body.main_color, (255 * i) /  my_line_cnt);
...
//				 #if !MYDESIGN
//						lv_lmeter_draw_scale(lmeter,&lmeter->coords);
//				 #endif
